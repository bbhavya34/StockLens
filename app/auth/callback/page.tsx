    ...
    <main className="flex min-h-screen items-center justify-center bg-background px-4 text-foreground">
      <div className="max-w-md rounded-2xl border border-border-strong bg-surface-1 p-8 text-center">
        {!error ? (
          ...
          <>
            <h1 className="font-display text-xl font-semibold">Sign-in could not be completed</h1>
            <p className="mt-2 text-sm text-muted-2">{error}</p>
            <Link href="/auth" className="mt-5 inline-block text-sm text-emerald hover:underline">Start a new sign-in</Link>
          </>
        )}
      </div>
    </main>
